...
public class Solution {
    ...
    public static void main(String[] args) throws IOException{
        ...
        while(query-->0){
            ...
            pawn    p
            */
            String ROOK="0000",BISHOP="0000",QUEEN="0000";
            String myrook = "00";
            String myking = "00";
            for(int i = 0; i<8; i++){
                for(int j = 0; j<8; j++){
                    ...
                    if(i<2 && r[i].charAt(j) == 'R')
                        ROOK += (8-i)+""+(j+1);

                    if(i<2 && r[i].charAt(j) == 'B')
                        BISHOP += (8-i)+""+(j+1);


                    if(i<2 && r[i].charAt(j) == 'Q')
                        QUEEN += (8-i)+""+(j+1);

                    if(i==1 && r[i].charAt(j) == 'r')
                        myrook = (8-i)+""+(j+1);
            ...
            for(int i = 0; i<8; i++){
                if(r[1].charAt(i)=='P'){
                    ppos += (i+1);
                    int c1 = rook(myking,(7+""+r[1].indexOf("r")),r);
                }
            }
